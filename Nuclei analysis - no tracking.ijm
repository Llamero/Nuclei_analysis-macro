...
while(piChannel > channelCount || piChannel == dapiChannel || piChannel < 1 || piChannel != round(piChannel) ) piChannel = getNumber("Which channel is the PI channel: 1-" + channelCount, 3);
...
//Create and array for saving each sample name
...
for(a=0; a<fileList.length; a++){
	...
	if(endsWith(fileList[a], ".nd2")){
		...
		run("Bio-Formats Importer", "open=[" + inputDirectory + fileList[a] + "] autoscale color_mode=Default view=Hyperstack stack_order=XYCZT");
		run("Split Channels");
...
updateResults();
saveAs("Results", inputDirectory + "Macro Results\\DAPI nuclei counts per frame.xls");
...
updateResults();
saveAs("Results", inputDirectory + "Macro Results\\PI nuclei counts per frame.xls");
...
saveAs("Tiff", inputDirectory + "Macro Results\\PI counts - Samples by row.tif");
close("PI counts - Samples by row.tif");
selectWindow("DAPI counts - Samples by row");
saveAs("Tiff", inputDirectory + "Macro Results\\DAPI counts - Samples by row.tif");
close("DAPI counts - Samples by row.tif");
selectWindow("Result of PI counts - Samples by row");
saveAs("Tiff", inputDirectory + "Macro Results\\PI ratio image.tif");
...
updateResults();
saveAs("Results", inputDirectory + "Macro Results\\PI ratio per frame.xls");
